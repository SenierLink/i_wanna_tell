...
之前一直用的index.php做的入口文件，突然想前后端分离，使用api.php作为api入口更为合理。

index.php配合router配合模板使用才会好很多。但是这次不打算用模板开发了。

#### 做个小总结吧

2019-12-13 18:54:36

正如之前所说，应该补充一些设计模式的知识，结合框架的思想，来开发这次后台。而不是直接硬上，什么都不知道完全是瞎写。

写的过程其实自己进步就很快了，频繁的修改结构。就是自己否定之前自己架构的结果。

使用composer才是真的上道了。nikic/fast-route确实简便，添加使用起来很快，可惜文档也确实不够新手向。看了我半天。